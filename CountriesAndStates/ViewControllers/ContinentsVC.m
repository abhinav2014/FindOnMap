...
@implementation ContinentsVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.fillColor = @"#FCD2A8";
    self.strokeColor = @"#EC6778";
    self.nowFillColor = @"#007D75";
    self.scrollViewColor = @"#ADD9FE";  // 529A86
...
-(void)configureView
{
    ...
    [self.navigationItem setTitle:@"Select a Continent"];
...
- (void)initIndiaMap
    {
        FSInteractiveMapView* map = [[FSInteractiveMapView alloc] initWithFrame:CGRectMake(0, 100, self.scrollView.frame.size.width, 600)];
        map.fillColor = [self colorWithHexString:self.fillColor];
        map.strokeColor = [self colorWithHexString:self.strokeColor];
        
        [map loadMap:@"continentsHigh" withColors:nil];
